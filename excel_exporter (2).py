...
Handles:
  ...
  - Automatic upload to GCS bucket after each export

Output directory defaults to `patent_exports/` next to this file,
or can be overridden via the EXCEL_OUTPUT_DIR env var.

GCS upload target: gs://<GCS_EXPORT_BUCKET>/patent_exports/
Uses SERVICE_KEY_PATH from .env for authentication.
"""
...
# GCS export config
GCS_EXPORT_BUCKET = os.getenv("GCS_EXPORT_BUCKET", "cognito-prod")
GCS_EXPORT_PREFIX = os.getenv("GCS_EXPORT_PREFIX", "patent_exports")
...
def _upload_to_gcs(local_path: str) -> Optional[str]:
    ...
    if not SERVICE_KEY_PATH:
        print("[GCS UPLOAD] SERVICE_KEY_PATH not set in .env — skipping GCS upload")
        return None
    ...
    try:
        ...
        blob_name = f"{GCS_EXPORT_PREFIX}/{filename}"
        blob = bucket.blob(blob_name)
        blob.upload_from_filename(local_path)

        gcs_uri = f"gs://{GCS_EXPORT_BUCKET}/{blob_name}"
        print(f"[GCS UPLOAD] ✓ Uploaded → {gcs_uri}")
        return gcs_uri

    except Exception as e:
        print(f"[GCS UPLOAD] Failed to upload {local_path}: {e}")
        return None
...
def export_to_excel(
    ...
    try:
        ...
        _upload_to_gcs(str(output_path.resolve()))
